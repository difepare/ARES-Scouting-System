...
# ==================================================
# FUNCIÓN PARA OBTENER ID DE UN EQUIPO POR SU NOMBRE
# ==================================================
def get_team_id(nombre):
    cur.execute("SELECT id FROM national_teams WHERE nombre = %s", (nombre,))
    result = cur.fetchone()
    if not result:
        print(f"⚠️ Equipo no encontrado: {nombre}")
        return None
    return result[0]

# ==================================================
# LISTA DE PARTIDOS (los 104 partidos del fixture)
# ==================================================
partidos = [
    # Grupo A
...
# ==================================================
print("🚀 Cargando los 104 partidos del Mundial 2026...\n")

# Limpiar tabla antes de insertar (opcional, comenta si quieres conservar datos)
...
for p in partidos:
    ...
    if local_id and visitante_id:
        try:
            ...
            insertados += 1
            print(f"✅ Partido {insertados}: {local} vs {visitante} ({fecha[:10]})")
        except Exception as e:
            print(f"❌ Error insertando {local} vs {visitante}: {e}")
    else:
        print(f"⚠️ No se pudo insertar {local} vs {visitante}")
